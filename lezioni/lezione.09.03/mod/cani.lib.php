<?php
     ...
     */
    function aggiungi($nome, $anno_nascita, $padrone) {
        $cani = lista();
     ...
     */
    function lista() {
        if (!file_exists(DB_CANI)) {
            return [];
        } else {
            return unserialize(file_get_contents(DB_CANI));
        }
    }
     ...
     */
    function modifica($id, $nome, $anno_nascita, $padrone) {
        $cani = lista();
        if (isset($cani[$id])) {
            $cani[$id] = [
                'nome' => $nome,
